...
object MarvelSuperherosAnalysis extends App {
  ...
  val sc = new SparkContext("local[*]", "MarvelSuperherosAnalysis")
  ...
  println()
  println(s"Top 10 most popular superheros with most co-appearances.")

  // Find the top 10 # of connections
  val topTen = flipped.sortByKey(ascending = false).take(10)

  val topTenPopularNamesWithCoAppearances = topTen.map(x => (namesRdd.lookup(x._2)(0), x._1))

  for (elem <- topTenPopularNamesWithCoAppearances) {
    println(s"${elem._1} with ${elem._2} co-appearances.")
  }
}
